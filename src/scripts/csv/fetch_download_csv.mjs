...
const classrooms = [
  {
    key: '19380377-2022-training-comp-on-open-source-os',
    assigments: [
...
function start_download_csv() {
  classrooms.map(classroom => {
    classroom.assigments.map(async assigment => {
      await download(classroom, assigment)
      console.log(`download ${assigment} csv finish!`)
    })
  })
}

async function download(classroom, assigment) {
  const url = `https://classroom.github.com/classrooms/${classroom.key}/assignments/${assigment}/download_grades`
